...
import UpdateDonationForm from "@/components/forms/UpdateDonationForm";
...
const UpdateCampaign = () => {
    ...
    const { data: donInfo, isLoading } = useQuery({
        queryKey: ["donationData", id],
        queryFn: async () => {
            const { data } = await axiosSecure(`${import.meta.env.VITE_API_URL}/donation-campaign-data/${id}`);
            return data;
        },
    });

    // Loading state: show animation if donInfo is not loaded
    if (!donInfo && isLoading) {
        return <UpdatePetSkeleton />;
    }

    if (!donInfo || typeof donInfo !== "object") {
        return (
            <div>
                <EmptyState />
            </div>
        ...
        <div className="bg-background-primary flex flex-col min-h-screen">
            ...
            <div>
                ...
                <div className="max-w-11/12 xl:max-w-8/12 mx-auto py-16">
                    <UpdateDonationForm donInfo={donInfo} />
                </div>
            </div>
        </div>
    );
