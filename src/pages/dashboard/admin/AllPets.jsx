...
import AdminAllPetsSkeleton from "@/components/Skeletons/AdminAllPetsSkeleton";
...
const AllPets = () => {
    ...
    const updateAdoptStatusMutation = useMutation({
        mutationFn: async ({ petId, adopted, adoption_status }) => {
            const body = adoption_status ? { adopted, adoption_status } : { adopted };
            return axiosSecure.patch(`/admin/adopt-status-update/${petId}`, body);
        },
        onSuccess: async () => {
            await refetch();
    ...
    const handleAdopt = async (pet) => {
        ...
        try {
            await updateAdoptStatusMutation.mutateAsync({ petId: pet._id, adopted: true });
        } finally {
            setAdoptLoading(null);
        }
    };

    // Don't Adopt pet handler
    const handleDontAdopt = async (pet) => {
        setAdoptLoading(pet._id);
        try {
            // Set adoption_status to 'pending' when Don't Adopt is clicked
            await updateAdoptStatusMutation.mutateAsync({ petId: pet._id, adopted: false, adoption_status: "pending" });
        } finally {
            setAdoptLoading(null);
        }
    };
    ...
    const handleDelete = async (petId) => {
        try {
            const { data } = await axiosSecure.delete(`/admin/dashboard/delete-pet/${petId}`);
            if (data?.success) {
                Swal.fire({
                    icon: "success",
                    title: "Deleted!",
                ...
                return (
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                navigate(`/admin/pet-update/${pet._id}`);
                            }}>
                            Update
                        </Button>
                        ...
                        <Button
                            ...
                            disabled={adoptLoading === pet._id}
                            onClick={(e) => {
                                e.stopPropagation();
                                handleAdopt(pet);
                            }}>
                            {adoptLoading === pet._id ? "Processing.." : "Adopt"}
                        </Button>
                        ...
                        <Button
                            ...
                            {adoptLoading === pet._id ? "Processing.." : "Don't Adopt"}
                        </Button>
                    </div>
                );
            },
    ...
    const table = useReactTable({
        ...
        pageCount: Math.ceil(allPets.length / 10),
        initialState: { pagination: { pageSize: 10 } },
    });

    if (isLoading) return <AdminAllPetsSkeleton />;
